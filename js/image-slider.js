let IdealImageSlider = (function() {
	"use strict";

	/*
	 * requestAnimationFrame polyfill
	 */
	let _requestAnimationFrame = function(win, t) {
	 ...
	 */
	let _requestTimeout = function(fn, delay) {
	...
	let _addClass = function(el, className) {
		if (!className) return;
		if (el.classList) {
	...
	let _removeClass = function(el, className) {
		if (!className) return;
		if (el.classList) {
	...
	let _toArray = function(obj) {
		return Array.prototype.slice.call(obj);
	};

	let _arrayRemove = function(array, from, to) {
		let rest = array.slice((to || from) + 1 || array.length);
		array.length = from < 0 ? array.length + from : from;
		return array.push.apply(array, rest);
	};

	let _addEvent = function(object, type, callback) {
	...
	let _loadImg = function(slide, callback) {
	...
	let _translate = function(slide, dist, speed) {
	...
	let _unTranslate = function(slide) {
	...
	let _setContainerHeight = function(slider, shouldAnimate) {
		...
		let currentHeight = Math.round(slider._attributes.container.offsetHeight),
		...
		if (slider._attributes.aspectWidth && slider._attributes.aspectHeight) {
			...
			// Auto
			let width = slider._attributes.currentSlide.getAttribute('data-actual-width');
			let height = slider._attributes.currentSlide.getAttribute('data-actual-height');
		...
		let maxHeight = parseInt(slider.settings.maxHeight, 10);
	...
	let _touch = {
		...
		move: function(event) {
			...
			let duration = +new Date() - _touch.vars.start.time;

			// Determine if slide attempt triggers next/prev slide
			let isChangeSlide = Number(duration) < 250 && Math.abs(_touch.vars.delta.x) > 20 || Math.abs(_touch.vars.delta.x) > this._attributes.currentSlide.offsetWidth / 2;

			let direction = _touch.vars.delta.x < 0 ? 'next' : 'previous';
			let speed = this.settings.transitionDuration ? this.settings.transitionDuration / 2 : 0;
		...
		transitionEnd: function(event) {
			if (_touch.vars.direction) {
				...
				let slides = this._attributes.slides,
	...
	let Slider = function(args) {
		...
		let sliderEl = document.querySelector(this.settings.selector);
		if (!sliderEl) return null;

		// Slides
		let origChildren = _toArray(sliderEl.children),
			validSlides = [];
		sliderEl.innerHTML = '';
		Array.prototype.forEach.call(origChildren, function(slide, i) {
			if (slide instanceof HTMLImageElement || slide instanceof HTMLAnchorElement) {
				let slideEl = document.createElement('a'),
					href = '',
					target = '';
		...
		if (!this.settings.disableNav) {
			...
			if (this.settings.keyboardNav) {
				_addEvent(document, 'keyup', function(e) {
					e = e || window.event;
					let button = (typeof e.which == 'number') ? e.which : e.keyCode;
					if (button == 37) {
						if (_hasClass(this._attributes.container, this.settings.classes.animating)) return false;
						this.stop();
						this.previousSlide();
		...
		if (_isInteger(this.settings.height)) {
			...
			if (this.settings.height.indexOf(':') > -1) {
				let aspectRatioParts = this.settings.height.split(':');
